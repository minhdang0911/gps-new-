...
export function getMadeInFromLabel({ value, mifOptions = [], isEn }) {
...
/** export excel */
export function exportDeviceCategoryExcel({ data, t, mifOptions, isEn }) {
    if (!data?.length) {
    ...
    try {
        const excelData = data.map((item) => ({
            ...
            Năm: item.year || '',
            Model: item.model || '',
            [t.columns.origin]: getMadeInFromLabel({ value: item.madeInFrom, mifOptions, isEn }),
